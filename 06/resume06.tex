...
% 説明入れて
map関数はxを範囲xMin〜xMaxから別の範囲yMin〜yMaxへ変換する関数です。
map関数を利用して、センサの値を実際の距離の計測値に変換してみましょう。
% 例えば、
 \begin{lstlisting}
 ...
 \begin{itemize}
\item	x	=	変換したい変数の名前
\item	xMin	=	センサで得られる下限値
\item	xMax	=	センサで得られる上限値
\item	yMin	=	変換したい（割り当てたい）変数の下限値
\item	yMax	=	変換したい（割り当てたい）変数の上限値
\end{itemize}
